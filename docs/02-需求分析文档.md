...
| v1.1 | 2026-05-28 | 关梓浩 | 跟踪实施进度（不改需求基线）：①§6.4 标注 `/v1/voice` 端到端能力升级为"含 LLM Tool Call + device_command 拦截执行 + 对话历史落库"；②§3.2 用例表 UC-01/02/04/05/06/10 主链路在 M3 已落地（详见 §9 跟踪矩阵）；③ UC-03/08/09/11/12/13 仍属计划中，演示期通过种子数据兜底；④ 非功能指标、用例规约、数据实体、领域模型等需求基线**保持不变** |
| v1.2 | 2026-05-29 | 关梓浩 | 跟踪实施进度（不改需求基线）：①§3.2 用例表 **UC-03 绑定家具设备** 与 **UC-11 解绑设备** 状态从"计划中"调整为"M3 已落地"——新增"待绑定池"模式（`devices.owner_id` 可空 + 6 位 `bind_code`），客户端右下角"添加设备"FAB → 绑定页 + 卡片长按 → 重命名/解绑；②§6.3 软件接口表新增 `/api/v1/devices/bindable`、`/api/v1/devices/bind`、`/api/v1/devices/{id}` PUT/DELETE 共 4 条 REST 端点；③ §5.1 数据实体 Device 新增 `bind_code` 属性、`owner_id` 改可空；④ 用例规约文字、非功能指标、领域模型等需求基线**保持不变**；⑤ 截至本次修订，13 个用例中 **UC-01/02/03/04/05（后端）/06/07/10/11 共 9 个已实现**，剩余 UC-05 客户端入口 / UC-08/09/12/13 仍为计划中。 |
| v1.3 | 2026-05-29 | 关梓浩 | 跟踪实施进度（不改需求基线）：①UC-05 客户端入口落地——Android 端新增"跟小菲聊聊"页面，输入文本即可控制设备并收到自然语言回复（复用 `/v1/voice` WS 通路 + 新增 `text_input` 事件，跳过 ASR 直接走 LLM Tool Call），与家具端语音指令共享同一套后端编排；②UC-02 登出闭环——AppBar 新增登出按钮，清本地 JWT 后跳登录页；③ 已实现用例数从 9 提升至 **10**（UC-01/02/03/04/05/06/07/10/11 主要演示用例全部覆盖），剩余 UC-08/09/12/13 仍为计划中。 |
| v1.4 | 2026-05-29 | 关梓浩 | UC-04 增强 + UC-10 实时推送落地，**UC-08 调整为产品形态淘汰**：①UC-04 体验补完——客户端新增设备详情页，按设备类型渲染亮度/温度+模式/开合度滑块，与语音控制并列形成"双控制路径"；②UC-10 落地——server 新增内存 Hub + `/ws?token=<jwt>` 端点，控制成功后向对应用户广播 `device_state_changed`，客户端原地翻转卡片（无需手动刷新）；③§3.2 用例表 **UC-08 历史会话**降级为"已被产品形态淘汰"——本系统对话本质是命令通道（用完即弃），保留 conversations/messages/commands 写入但**不开放查询 API**，把工时投入演示价值更高的实时推送；④ 已实现用例数 **11**（UC-01~07/10/11，UC-08 不计入），剩余 UC-09/12/13。 |
...
本文档面向项目全体成员、课程考核组及未来维护人员，清晰、完整、可验证地描述"智能家居语音交互助手系统（Taffy）"的功能需求与非功能需求，作为后续软件设计、编码实现与系统测试的基线。
...
| 大模型工作流 | Go Worker 进程中的 LLM 编排模块，负责调用云端 LLM API（支持 Function Calling / Tool Call）进行意图识别与设备控制；外加本地 ASR/TTS 服务的调用编排 |
...
Taffy 是一套面向个人/家庭用户的"语音交互式"智能家居控制系统，覆盖**家具端 — 服务器 — 客户端**三层协同，以 Android App 作为用户入口，以大模型为"语义大脑"（由 Go 服务器直接编排），以 Go 服务器为消息中枢，最终控制家具端设备。
...
| 服务器 | Linux (Ubuntu 22.04) / Windows 10+；Go 1.22+ |
| 本地 ASR 服务 | Python 3.10+；FunASR paraformer-zh-streaming + fsmn-vad；CPU/GPU 均可；默认端口 9100 |
| 本地 TTS 服务 | Python 3.10+；Piper；CPU 即可超实时；默认端口 9200 |
...
2. ASR 与 TTS 采用**本地独立部署的微服务**，对外提供 HTTP/WS API，供 Go Worker 调用；LLM 使用**第三方云端 API**（通义/豆包/DeepSeek/GLM 等），由 Go Worker 通过 `config.yaml` 配置的 URL + 环境变量 `LLM_API_KEY` 调用，支持 OpenAI Function Calling（Tool Call）。
...
| UC-05 | 文本指令控制设备 | Must | 用户 | 客户端、服务器、家具端 |
| UC-06 | 语音指令控制设备 | Must | 用户 | 客户端、服务器、家具端 |
...
| **用例名称** | 语音指令控制设备 |
| **优先级** | Must |
| **主要角色** | 用户；协作：服务器（含 LLM 编排）、家具端 |
| **前置条件** | 已绑定至少一台设备；家具端（带麦）在线 |
| **后置条件** | 目标设备状态变更；会话被记录；可选语音反馈 |
| **主成功场景** | 1. 用户对着家具端（带麦音箱"小菲"）说话<br>2. 家具端 VAD 检测到语音开始，通过 WS 发送 `start` + PCM 音频流到 Go 服务器<br>3. Go 服务器将音频原样透传给 Worker，Worker 再透传给本地 ASR 服务（WS 流式识别）<br>4. ASR 返回 `partial` 中间结果，经 Worker/Server 透传为 `asr_partial` 给家具端<br>5. 用户说完停顿 ≥ 800ms，家具端 VAD 自动发 `end` 并**暂停语音检测**<br>6. ASR 返回 `final` 文本，经 Worker/Server 透传为 `asr_final` 给家具端<br>7. Worker 将文本 + 设备上下文 + Tool Schema 发送给云端 LLM API<br>8. 若 LLM 判断需要控制设备，返回 `tool_calls`（如 `control_device`）→ Worker 下发 `device_command` 给 Server → Server 执行设备控制（更新 DB）→ 回复 `device_command_result` → Worker 二次调用 LLM → LLM 生成自然语言回复<br>9. 若 LLM 无需调工具，直接返回文本回复<br>10. Worker 将最终回复通过 `llm_result` 事件推回 Server → Server 透传给家具端<br>11. 家具端收到 `llm_result`，输出文字，**恢复语音检测**，等待下一轮对话 |
| **扩展场景** | **4a. ASR 失败/空文本**：Worker 发空 `llm_result`，家具端恢复检测<br>**7a. LLM 不可达**：返回 `llm_error`，家具端打印提示并恢复检测<br>**8a. 设备控制失败**：Server 回传 `device_command_result(success=false)`，Worker 二次调用 LLM 时携带失败信息，LLM 生成"抱歉，操作失败"类回复 |
| **特殊需求** | 端到端（VAD 发 end → 收到 llm_result）响应时间 ≤ 3 秒（NFR-P1）；LLM API Key 通过环境变量 `LLM_API_KEY` 配置 |
...
| UC-05 文本指令 | 文本消息 | 与 UC-06 共用 LLM，跳过 ASR | 同 UC-06 的 7a |
...
| Device | 家具设备 | device_id, owner_id（**可空**：NULL 表示位于待绑定池）, device_type, name, room, status, **bind_code**（6 位绑定码，绑定后清空）, last_online_at |
| DeviceState | 设备实时状态 | device_id, power, brightness, temperature, mode, position |
...
| 通道 | 协议 | 用途 |
|---|---|---|
| 客户端 ↔ 服务器（REST） | HTTPS/JSON | 登录、设备管理（含 **UC-03 绑定 / UC-11 解绑 / 重命名**：`/api/v1/devices/bindable`、`/api/v1/devices/bind`、`/api/v1/devices/{id}` PUT/DELETE）等同步请求（M3） |
| 客户端 ↔ 服务器（WebSocket） | WSS | 实时状态推送、会话流（M3） |
| **家具端 ↔ 服务器（音频）** | **WSS /v1/voice** | **音频上行、asr_partial/asr_final 下行、llm_result 下行（M2 已实现）** |
| 服务器 ↔ ASR 服务 | WS /v1/asr/stream | 流式语音识别（透传家具端音频） |
| Worker ↔ TTS 服务 | HTTP /v1/tts/synthesize | 语音合成（Worker 异步调用，结果以 `tts_audio` 下发家具端） |
| 服务器 ↔ 云端 LLM API | HTTPS（JSON） | 文本 + 设备上下文 + Tool Schema → Function Calling 指令（由 Worker 调用） |
| 服务器 ↔ 家具端（控制） | MQTT v3.1.1 | 指令下发（`taffy/device/{id}/cmd`）+ 状态/心跳/结果上行（`status`/`heartbeat`/`result`），broker 配置为空时整个 bridge 禁用 |
...
:用户对着家具端说话;
:家具端 VAD 检测到语音开始;
:家具端发送 start + PCM 流到 Go 服务器;
:Go 服务器透传给 Worker → ASR;

if (ASR 识别成功?) then (是)
  :得到转写文本 (asr_final);
else (否)
  :Worker 发空 llm_result;
  :家具端恢复检测;
  stop
endif
...
:家具端 VAD 检测到静音≥800ms;
:家具端发 end 并暂停语音检测;
:Worker 将文本+设备上下文+Tool Schema 发往云端 LLM;

if (LLM 返回?) then (tool_calls)
  :Worker 下发 device_command 给 Server;
  :Server 执行设备控制（更新 DB）;
  :Server 回复 device_command_result;
  :Worker 二次调用 LLM（携带 tool 结果）;
  :LLM 生成自然语言回复;
elseif (LLM 返回?) then (纯文本)
  :直接得到回复文本;
else (失败)
  :Worker 发 llm_error;
  :家具端恢复检测;
