...
| 大模型工作流 | Go 服务器内部的 LLM 编排模块，负责调用云端 LLM API 进行意图识别与结构化指令生成；外加本地 ASR/TTS 服务的调用编排 |
...
2. ASR 与 TTS 采用**本地独立部署的微服务**，对外提供 HTTP/WS API，供 Go 服务器调用；LLM 使用**第三方云端 API**（通义/豆包/DeepSeek 等），由 Go 服务器通过 `config.yaml` 配置的 URL + 环境变量 `LLM_API_KEY` 直接调用。
...
| **主成功场景** | 1. 用户对着家具端（带麦音箱"小菲"）说话<br>2. 家具端 VAD 检测到语音开始，通过 WS 发送 `start` + PCM 音频流到 Go 服务器<br>3. Go 服务器将音频原样透传给本地 ASR 服务（WS 流式识别）<br>4. ASR 返回 `partial` 中间结果，Go 服务器转发为 `asr_partial` 给家具端<br>5. 用户说完停顿 ≥ 800ms，家具端 VAD 自动发 `end` 并**暂停语音检测**<br>6. ASR 返回 `final` 文本，Go 服务器转发为 `asr_final` 给家具端<br>7. Go 服务器将文本 + 设备上下文发送给云端 LLM API，等待结构化 JSON 指令与回复<br>8. LLM 返回后，Go 服务器将结果通过 `llm_result` 事件推回家具端<br>9. 家具端收到 `llm_result`，输出文字，**恢复语音检测**，等待下一轮对话 |
| **扩展场景** | **4a. ASR 失败/空文本**：Go 服务器发空 `llm_result`，家具端恢复检测<br>**7a. LLM 不可达**：返回 `llm_error`，家具端打印提示并恢复检测 |
...
| 服务器 ↔ 云端 LLM API | HTTPS（JSON） | 文本 + 设备上下文 → 结构化指令（M2 已实现） |
