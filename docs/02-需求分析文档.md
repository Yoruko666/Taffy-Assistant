...
| 项目名称 | 智能家居语音交互助手系统 |
|---|---|
| 项目代号 | SmartHome-Voice-Assistant（Taffy） |
| 文档版本 | v1.0 |
| 文档状态 | 草案（Draft） |
| 编写人 | 关梓浩 |
...
| 服务器 ↔ TTS 服务 | HTTP /v1/tts/synthesize | 语音合成（M3） |
| 服务器 ↔ 云端 LLM API | HTTPS（JSON） | 文本 + 设备上下文 + Tool Schema → Function Calling 指令（由 Worker 调用，v0.5 已实现） |
| 服务器 ↔ 家具端（控制） | MQTT v3.1.1 | 指令下发、状态上报（M3） |
