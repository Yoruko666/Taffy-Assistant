...
| 设备 | `/api/v1/devices/pending` / `/devices/bind` / `/devices/{id}/unbind` | — | JWT | UC-03 / UC-11 | 📅 计划中（演示期手动 insert `devices`+`device_states`） |
...
### 6.1 设备绑定（UC-03，**端点 `/devices/pending` / `/devices/bind` 计划中**）

> 现状：演示期通过 `server/migrations/002_seed.sql` 直接插入 `devices` + `device_states` 表完成"出厂预绑定"。下述 `POST /devices/bind` 等端点为下一迭代规划，时序图描述目标形态，便于后续开发对齐。
...
Srv -> Srv : 写入归属关系
Srv -> MQ  : PUBLISH taffy/device/{id}/cmd (bound)
MQ -> Dev  : 通知已绑定
Srv --> App : 绑定成功
@enduml
```
