...
1. **音频输入端是家具端，不是 App**：用户对着"带麦家具"（类似小度音箱）说话，家具端通过 WebSocket 把 PCM 流发给 Go Server。
...
| 子类 | 典型设备 | 通道 | 职责 |
|---|---|---|---|
| 带麦家具 | 智能音箱（类小度）、网关 | WS `/v1/voice` + MQTT | 麦克风录音 → 16k/16-bit/mono PCM → WS 上传；接收 `asr_partial / asr_final / reply / tts_audio`；播放 TTS；同时受 MQTT 控制 |
