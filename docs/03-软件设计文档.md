...
3. **客户端 App 不录音**：仅做登录、设备管理、状态查看、场景触发、对话历史，通过 REST + WSS 与 Go Server 通信。M2 已搭建基础框架（Kotlin + Compose + OkHttp WebSocket）。
4. **ASR 走 WebSocket 流式**：`ws://asr:9100/v1/asr/stream`，Go Server 在 `/v1/voice` 端点对家具端音频做透明转发。
5. **LLM 配置通过 `worker/config.yaml` + 环境变量 `LLM_API_KEY`**：Worker 启动时加载，API URL、Model、System Prompt 等均可在配置文件中指定，API Key 支持环境变量覆盖以保护敏感信息。v0.5 起 Worker 使用 **OpenAI Function Calling** 模式，LLM 按需调用 `control_device` / `activate_scene` 工具函数。
...
**关键契约（LLM Tool Calling）**：v0.5 起，LLM 采用 **OpenAI Function Calling** 模式，不再使用自定义 JSON Schema 输出。Worker 在调用 LLM 时携带 `tools` 参数，LLM 按需返回 `tool_calls`。
...
- `001_init.sql`：建库 `shva` + 7 张表（users/devices/device_states/conversations/messages/commands/scenes）
- `002_seed.sql`：2 个测试用户 + 9 台设备 + 对应状态 + 2 个场景
...
| 业务逻辑 | `internal/service/` | 密码校验、权限控制、状态合并 |
| HTTP 处理 | `internal/handler/` | 请求解析、响应序列化 |
...
> **v0.5 实现范围**：家具端 → ASR → Server 透传 → Worker → LLM Tool Call → Server 执行设备控制 → Worker 二次 LLM → `llm_result` 回家具端。TTS 与 MQTT 部分为 M3 待实现。
