...
| 认证 | `/api/v1/auth/logout` | POST | JWT | UC-02 | 📅 计划中（Redis JWT 黑名单底座已就绪）|
| 用户 | `/api/v1/users/me` | GET / PATCH | JWT | UC-12 | 📅 计划中 |
| 用户 | `/api/v1/users/me/password` | POST | JWT | UC-12 | 📅 计划中 |
| 用户 | `/api/v1/users/me` | DELETE | JWT | UC-13 | 📅 计划中 |
| 会话 | `/api/v1/conversations` / `/conversations/{id}/messages` | GET | JWT | UC-08 | 📅 计划中（**写入**路径已完成，由 `/v1/voice` 同步落库 `conversations / messages / commands`，**查询** API 待开放）|
| 场景 | `/api/v1/scenes` / `/scenes/{id}/trigger` | — | JWT | UC-09 | 📅 计划中（`scenes` 表与 `activate_scene` tool schema 已声明）|
| 状态推送 | `/ws`（客户端实时推送 Hub）| WS | JWT | UC-10 | 📅 计划中 |
