...
| 模块 | 所属端 | 核心职责 | 负责人 |
|---|---|---|---|
| M-CL Android 客户端 | 客户端 | UI、录音、网络通信、状态展示 | 马正朗 |
| M-GW API 网关 | 服务器 | 路由、鉴权、限流、日志 | 林帅 |
| M-AU 认证服务 | 服务器 | 注册、登录、JWT 签发/校验 | 关梓浩 + 林帅 |
| M-DV 设备服务 | 服务器 | 设备绑定、状态缓存、MQTT 收发 | 林帅 |
| M-CH 会话服务 | 服务器 | 会话/消息持久化、WebSocket 推送 | 林帅 |
| M-SC 场景服务 | 服务器 | 场景定义与触发 | 林帅 |
| M-LM 大模型工作流 | 工作流 | 编排 ASR/LLM/TTS，产出结构化指令 | 吴承凯 |
| M-AS 本地 ASR 服务 | 工作流 | 对外提供语音识别 HTTP API | 吴承凯 |
| M-TS 本地 TTS 服务 | 工作流 | 对外提供语音合成 HTTP API | 吴承凯 |
| M-DE 家具端 | 家具端 | 设备驱动/模拟、MQTT 通信 | 刘智冲 |
