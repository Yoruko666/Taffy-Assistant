...
5. **LLM 配置通过 `server/config.yaml` + 环境变量 `LLM_API_KEY`**：Go Server 启动时加载，API URL、Model、System Prompt 等均可在配置文件中指定，API Key 支持环境变量覆盖以保护敏感信息。
...
> **M2 实现范围**：家具端 → ASR → Go Server → LLM → `llm_result` 回家具端，含 VAD pause/resume。TTS 与 MQTT 部分为 M3 待实现。
...
Srv --> Mic : {"type":"llm_result","text":"好的，已为您打开客厅灯"} (M2)
