...
整体形态对齐"小度 / 小爱 / Alexa"的端云分工：

- **家具端 = 本地"小度硬件"**：跑在音箱 / PC 模拟器上，负责 **录音 → KWS 唤醒词 → VAD 端点检测 → WS 上行 → 播放 TTS**。**断句在端侧做**，不靠云端。
...
│
├── furniture/                                      # 家具端（像小度一样：KWS + VAD + WS 长连接）
...
用 `furniture/mock_furniture.py` 扮演"带麦克风的小度音箱"，用 wav 文件作为虚拟麦克风。脚本内置 KWS 抽象层（M2 默认 AlwaysOn）+ webrtcvad 端点检测，能像真机一样自动断句、一条 WS 多句连说。
